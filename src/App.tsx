...
export default function App() {
  ...
  const handleUploadCustomFile = async (file: File, name: string, niche: any, description: string, rawTranscribe: string) => {
    ...
    try {
      const result = await runAnalyzeVideo({
        ...
        videoFile: file,
        videoUrl // used locally
      });
      setActiveProject({ ...result.project, id: `custom-${Date.now()}`, videoUrl, name });
      setActiveTab('studio');
    } catch (err: any) {
  ...
  const triggerExport = async () => {
    ...
    try {
      ...
      setDownloadReadyInfo({ url: URL.createObjectURL(editedBlob), filename: `${activeProject.name}_edited.mp4` });
    } catch (err: any) {
  ...
  if (!hasStarted) {
    return (
      <div className="min-h-screen bg-[#020617] flex flex-col items-center justify-center p-8 text-center">
        ...
        <h1 className="text-2xl font-black text-white uppercase tracking-tighter mb-4 text-gradient bg-gradient-to-tr from-purple-400 to-pink-500 bg-clip-text text-transparent">Viral AI Editor</h1>
          ...
          <div className="space-y-6">
            <div className="flex flex-wrap gap-2 justify-between items-center bg-slate-900/80 p-4 rounded-3xl border border-slate-800">
               ...
               <div className="flex gap-2">
                 ...
                 <button onClick={triggerExport} className="bg-purple-600 hover:bg-purple-500 px-6 py-2 rounded-xl font-black text-[10px] uppercase flex items-center gap-2">
                   <Download className="w-3.5 h-3.5" /> Bake
                 </button>
               </div>
            </div>
            
            <Suspense fallback={<div className="p-20 text-center text-slate-500 font-mono text-xs uppercase tracking-widest">Warming Engines...</div>}>
              {activeTab === 'studio' ? (
