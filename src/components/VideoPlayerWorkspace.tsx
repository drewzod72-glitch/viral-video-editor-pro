...
export default function VideoPlayerWorkspace({
  ...
  const [likeCount, setLikeCount] = useState(1284501);
  ...
  const handleTimeUpdate = () => {
    ...
    // Smart-cuts Highlights compilation
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
      <div className="lg:col-span-3 flex flex-col bg-slate-950 border border-slate-900 rounded-2xl overflow-hidden shadow-2xl relative">
        ...
        <div id="video-preview-stage" ref={stageRef} className="relative aspect-[9/16] h-[500px] max-h-[60vh] max-w-[281px] mx-auto flex items-center justify-center bg-zinc-950 rounded-2xl overflow-hidden shadow-[0_8px_48px_rgba(0,0,0,0.95)] border border-slate-900/80 my-4">
          <video ref={videoRef} src={project.videoUrl} onTimeUpdate={handleTimeUpdate} onLoadedMetadata={handleLoadedMetadata} playsInline style={{ transform: `scale(${currentZoomScale})` }} className={`w-full h-full object-cover transition-all duration-300 ${enableColorGrade && project.colorGrade !== 'none' ? `filter-${project.colorGrade}` : ''}`} />
          ...
          {!isPlaying && <div onClick={togglePlay} className="absolute inset-0 bg-slate-950/45 flex items-center justify-center cursor-pointer group z-10"><div className="w-13 h-13 rounded-full bg-brand-purple text-white flex items-center justify-center shadow-2xl transform group-hover:scale-110 transition-all"><Play className="w-5.5 h-5.5 fill-white" /></div></div>}
        </div>
        ...
        <div className="bg-slate-900/60 p-4 border-t border-slate-900 backdrop-blur-md">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => { videoRef.current!.currentTime = startLimit; setCurrentTime(startLimit); }} className="p-2.5 rounded-xl bg-slate-800 text-slate-300"><RotateCcw className="w-4 h-4" /></button>
            </div>
            <div className="flex flex-wrap items-center bg-slate-950 p-1 rounded-xl border border-slate-800/80 gap-1">
              ...
              {project.highlights.length > 0 && <button onClick={() => onClipSelect('smart-cuts')} className={`px-3 py-1 rounded-lg text-xs font-bold ${activeClipId === 'smart-cuts' ? 'bg-brand-purple text-white shadow-lg shadow-brand-purple/20' : 'text-brand-cyan hover:text-cyan-300'}`}>AI Cuts</button>}
            </div>
          </div>
        </div>
      </div>

      <div className="space-y-4 overflow-y-auto max-h-[80vh] custom-scrollbar">
        ...
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl">
          ...
          <input type="text" placeholder="Search feeling..." value={musicSearchQuery} onChange={(e) => setMusicSearchQuery(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2 text-xs focus:border-brand-cyan focus:outline-none mb-3" />
          <div className="max-h-[200px] overflow-y-auto space-y-1.5 custom-scrollbar">
            {filteredMusic.map(t => (
              <button key={t.id} onClick={() => updateSettings({ selectedMusicTrackId: t.id })} className={`w-full text-left p-2.5 rounded-xl border transition-all ${project.selectedMusicTrackId === t.id ? 'border-brand-cyan bg-brand-cyan/10 text-brand-cyan' : 'border-transparent hover:bg-slate-800/40 text-slate-200'}`}><div className="text-xs font-bold truncate">{t.name}</div></button>
            ))}
          </div>
        ...
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl space-y-4 font-sans">
          ...
          {isAnalyzing ? (
            <div className="bg-slate-950 p-4 rounded-xl border border-brand-cyan/25 space-y-3 relative overflow-hidden"><div className="flex-1"><h4 className="text-xs font-semibold text-slate-200">Analyzing...</h4><div className="w-full bg-slate-900 rounded-full h-1.5 mt-2 overflow-hidden"><div className="bg-brand-purple h-1.5 transition-all duration-300" style={{ width: `${(analyzeStep + 1) * 20}%` }} /></div></div></div>
          ) : (
